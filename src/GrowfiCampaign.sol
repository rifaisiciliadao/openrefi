...
/// @title GrowfiCampaign — Token Sales, Escrow, Sell-Back Queue, Buyback
...
contract GrowfiCampaign is Initializable, ReentrancyGuard, PausableUpgradeable {
    ...
    ///         future GrowfiCampaign-level fees. Snapshotted from the factory at init.
    ...
    ///         separately in `GrowfiHarvestManager.protocolFeeBps`. On fresh campaigns
    ...
    ///         `GrowfiCampaign.metadataURI → productType`. Immutable after init.
    ...
    /// @notice Owning GrowfiHarvestManager. Wired post-deploy via `setHarvestManager`.
    GrowfiHarvestManager public harvestManager;
    ...
    // --- Appended storage (v4 — GROW token integration) ---

    /// @notice GrowfiMinter contract that gates GROW emission on this campaign's buys.
    ///         Snapshotted from `factory.growfiMinter()` at init. If address(0), no GROW
    ///         is emitted (legacy / bootstrap window). Hooks fired in buy(), _activate(),
    ///         triggerBuyback() — all conditionals on `growMinter != address(0)`.
    address public growMinter;
    ...
    function initialize(InitParams calldata p) external initializer {
        ...
        // Snapshot the GROW minter address from the factory if the GROW system is wired.
        // If the factory returns 0 (or the call fails — older factory impl), grow integration
        // is skipped silently and the campaign runs as a standalone (no GROW emission).
        if (p.factory != address(0)) {
            try IGrowfiCampaignFactoryView(p.factory).growfiMinter() returns (address gm) {
                if (gm != address(0)) growMinter = gm;
            } catch {
                // Factory doesn't expose growfiMinter() — pre-GROW factory impl. Skip.
            }
        }
    }

    /// @notice Set the GrowfiCampaignToken address. Can only be called once by the factory.
    ...
    /// @notice Wire the GrowfiStakingVault. Called by factory during setup.
    ...
    /// @notice Wire the GrowfiYieldToken on the GrowfiStakingVault. Called by factory during setup.
    function setYieldToken(address yieldToken_) external onlyFactory {
        stakingVault.setYieldToken(yieldToken_);
    }

    /// @notice Wire the owning GrowfiHarvestManager. Called once by the factory after
    ///         the GrowfiHarvestManager proxy is deployed. Required for
    ...
    function setHarvestManager(address harvestManager_) external onlyFactory {
        ...
        harvestManager = GrowfiHarvestManager(harvestManager_);
    ...
    ///         `GrowfiHarvestManager.depositUSDC` path. The GrowfiHarvestManager's
    ...
    ///         the wallet portion before calling. The GrowfiCampaign then
    ...
    ///         GrowfiHarvestManager so holders can `claimUSDC` normally. Marks the
    ...
    // --- GrowfiCampaign parameter updates ---
    //
    ...
    function buy(address paymentToken, uint256 paymentAmount) external nonReentrant whenNotPaused {
        ...
        // Capture supply BEFORE any mint so the GROW bonding-curve hook can compute the
        // exact delta after the mint. Queue fills are supply-neutral (burn+mint), so
        // the hook only triggers GROW emission for fresh mints.
        ...
        // GROW emission hook: fires AFTER the buy logic, BEFORE auto-activate.
        // The minter applies the bonding curve over (supplyBefore → currentSupply) — only
        // fresh mints contribute (queue fills are supply-neutral). If the campaign hasn't
        // reached softcap yet, the GROW lands in escrow; once _activate() fires below,
        // the escrow becomes claimable.
        if (growMinter != address(0)) {
            IGrowfiMinter(growMinter).recordBuy(msg.sender, supplyBefore, currentSupply);
        }
    ...
    function triggerBuyback() external {
        ...
        // GROW hook: campaign failed → void all per-user escrow for this campaign.
        if (growMinter != address(0)) {
            IGrowfiMinter(growMinter).onBuyback();
        }
    ...
    function _activate() internal {
        ...
        // GROW hook: campaign reached softcap → unlock escrow for claim.
        if (growMinter != address(0)) {
            IGrowfiMinter(growMinter).onSoftCapReached();
        }
