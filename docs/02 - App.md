...
| **Staking** | Stake/unstake $CAMPAIGN, manage multiple positions, live $YIELD accrual counter, withdraw $YIELD, current yield rate (5→1 gauge), penalty calculator per position |
...
| **Create Campaign** | Producer form: token price, min cap, max cap (auto-calculated from # of assets × 1,000), funding deadline, season duration (≥ 1yr), min product claim amount |
...
- Unstake: instant return with penalty preview per position
