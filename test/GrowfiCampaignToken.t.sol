...
contract CampaignTokenTest is Test {
    GrowfiCampaignToken token;
    address campaign = address(0x1);
    address vault = address(0x2);
    address user = address(0x3);

    function setUp() public {
        GrowfiCampaignToken impl = new GrowfiCampaignToken();
        bytes memory initData = abi.encodeCall(GrowfiCampaignToken.initialize, ("Olive Tree", "OLIVE", campaign));
        token = GrowfiCampaignToken(address(new TransparentUpgradeableProxy(address(impl), address(this), initData)));
        vm.prank(campaign);
